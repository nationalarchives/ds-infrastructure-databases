...
module "postgres-main-prime" {
    ...
    secret_id = "/infrastructure/credentials/postgres-main"
...
module "postgres-main-replica" {
    source = "./postgres-main"

    count = var.postgres_main_replica == true ? 1 : 0
    ...
    instance_type = var.postgres_main_instance_type
    volume_size   = var.postgres_main_volume_size
    key_name      = var.postgres_main_replica_key_name
    ...
    attached_ebs_volume_id = data.aws_ssm_parameter.postgres_main_replica_volume_id.value
    ...
    zone_id           = data.aws_ssm_parameter.route53_zone_id.value
    postgres_main_dns = var.postgres_main_replica_dns
